...
<div class="relative flex flex-col bg-gray-100 dark:bg-gray-900 {isGameplayPage ? 'h-[100dvh] overflow-hidden' : 'min-h-screen'}">
  {#if $isLoggedIn && $location.pathname !== `${base}/login`} 
    <Header />
    <SideMenu />
  {/if}

  <main class={isGameplayPage ? 'flex-1 overflow-hidden relative' : 'p-4 sm:p-6'}>
    {#if !$isLoggedIn}
      <Login />
    {:else}
