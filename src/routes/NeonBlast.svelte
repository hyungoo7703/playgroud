...
<div class="flex flex-col items-center w-full h-[100dvh] md:h-auto md:min-h-screen bg-[#050505] text-white select-none font-sans relative overflow-hidden md:overflow-y-auto">
  ...
  {#if !isStarted}
    <div class="flex flex-col items-center justify-center min-h-[100dvh] w-full px-6 py-20 z-10">
      <h1 class="text-6xl md:text-7xl font-black text-cyan-400 drop-shadow-[0_0_20px_#22d3ee] mb-16 italic text-center">NEON BLAST</h1>
  ...
  {:else}
    <div class="flex flex-col items-center w-full max-w-[500px] h-full md:h-auto min-h-full">
      
      <header class="w-full px-5 py-4 flex justify-between items-center bg-gray-900/60 backdrop-blur-md border-b border-white/10 shrink-0 z-20">
        <div class="flex flex-col">
          <span class="text-[11px] text-cyan-400 font-bold uppercase tracking-widest leading-none">Stage {$gameStore.currentLevel}</span>
          <p class="text-3xl font-black italic text-white mt-1">{$gameStore.ballsLeft}</p>
        </div>
        ...
        <div class="text-right flex flex-col">
          <span class="text-[11px] text-magenta-500 font-bold uppercase tracking-widest leading-none">Score</span>
          <p class="text-3xl font-black italic text-white mt-1">{$gameStore.score}</p>
        </div>
      </header>

      <main class="flex-1 w-full flex flex-col items-center justify-center p-2 min-h-0 relative">
        <div class="my-3 text-center shrink-0 z-10">
          <p class="text-yellow-400 font-black text-lg md:text-xl animate-pulse">✨ 모든 황금 핀을 제거하면 클리어! ✨</p>
        </div>
        
        <div class="relative w-full h-full max-h-[60dvh] md:max-h-none aspect-[2/3] touch-none">
          <canvas bind:this={canvas} width="400" height="600" 
            ...
            class="w-full h-full rounded-[2.8rem] border-4 border-[#00f3ff] shadow-[0_0_40px_rgba(0,243,255,0.2)]"
      ...
      <footer class="w-full py-6 px-8 flex justify-center items-center shrink-0 z-20">
